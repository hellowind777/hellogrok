...
type Target struct {
	...
	// ReasoningEffortDefault is projected only when neither a per-model nor a
	// global selection exists. Bare-string menus otherwise default to their
	// first entry, unlike the legacy generated menu whose default was High.
	ReasoningEffortDefault string
	// MigrateLegacyReasoningMenu compacts the exact object menu generated by
	// older releases and keeps the compact equivalent after proxy shutdown.
	MigrateLegacyReasoningMenu bool
...
func ApplyTargets(configPath, statePath string, targets []Target) (ApplyResult, error) {
	...
	for _, target := range targets {
		...
		target.ReasoningEffortDefault = strings.ToLower(strings.TrimSpace(target.ReasoningEffortDefault))
		if target.ReasoningEffortDefault != "" {
			found := false
			for _, option := range target.ReasoningEfforts {
				found = found || option.Value == target.ReasoningEffortDefault
			}
			if !found {
				return ApplyResult{}, fmt.Errorf("model %q reasoning default %q is not in its menu", target.ID, target.ReasoningEffortDefault)
			}
		}
...
func validateManagedConfig(raw []byte, targets map[string]Target, state State) error {
	...
	for _, id := range ids {
		...
		if expected := targets[id].ReasoningEfforts; len(expected) > 0 {
			...
			if expectedDefault := targets[id].ReasoningEffortDefault; expectedDefault != "" {
				actualDefault, _ := model["reasoning_effort"].(string)
				if strings.ToLower(strings.TrimSpace(actualDefault)) != expectedDefault {
					return fmt.Errorf("[model.%s].reasoning_effort must be %q", id, expectedDefault)
				}
			}
		}
	}
...
func reasoningEffortsTOML(options []ReasoningEffortOption) string {
	...
	for _, option := range options {
		entries = append(entries, quoteTOML(option.Value))
	}
...
func rewriteModelBlock(block []string, id string, target Target, state *State, ending string, result *ApplyResult) ([]string, error) {
	...
	if target.ReasoningEffortDefault != "" &&
		...
		if target.MigrateLegacyReasoningMenu && !modelState.ReasoningEffort.Managed &&
			!modelBlockHasField(block, reasoningEffortAnyLine) {
			modelState.ReasoningEffort = ManagedLineState{
				Managed:          true,
				Present:          true,
				OriginalLine:     "reasoning_effort = " + quoteTOML(target.ReasoningEffortDefault) + ending,
				PreApplyRecorded: true,
...
func rewriteExistingManagedCompositeField(
	...
	if start, end := reasoningEffortArrayTableSpan(block, firstContent, modelID); start >= 0 {
		replacement := field.name + " = " + field.value + lineEnding(block[end-1])
		if lineEnding(block[end-1]) == "" {
			replacement += ending
		}
		if !field.state.Managed {
			field.state.Managed = true
			field.state.Present = true
			if migrateLegacy {
				if err := recordPreApplyModelField(block, modelID, field.name, field.state); err != nil {
					return nil, err
				}
				field.state.OriginalLine = replacement
			} else {
				field.state.OriginalLine = strings.Join(block[start:end], "")
			}
		}
		field.state.AppliedValue = managedSemanticValue(field.value)
		next := make([]string, 0, len(block)-(end-start)+1)
...
func recordPreApplyModelField(block []string, modelID, key string, state *ManagedLineState) error {
	var root map[string]any
	if err := toml.Unmarshal([]byte(strings.Join(block, "")), &root); err != nil {
		return fmt.Errorf("parse pre-migration model block: %w", err)
	}
	model := ModelTables(root)[modelID]
	value, exists := model[key]
	state.PreApplyRecorded = true
	state.PreApplyPresent = exists
	if !exists {
		return nil
	}
	semantic, ok := managedScalarString(value)
	if !ok {
		return fmt.Errorf("pre-migration %s has an unexpected type", key)
	}
	state.PreApplyValue = semantic
	return nil
}

func reasoningEffortArrayTableSpan(block []string, firstContent int, modelID string) (int, int) {
	quotedID := regexp.QuoteMeta(modelID)
	header := regexp.MustCompile(`^\s*\[\[\s*model\.(?:` + quotedID + `|"` + quotedID + `"|'` + quotedID + `')\.reasoning_efforts\s*\]\]\s*(?:#.*)?$`)
	field := regexp.MustCompile(`^\s*(?:value|label|default)\s*=`)
	structural := tomlStructuralLines(block)
	start, end := -1, -1
	for index := firstContent; index < len(block); index++ {
		if !structural[index] {
