...
Grok Build can use custom model endpoints, but real-world providers do not all expose the same protocol, response shape, authentication method, tool-call names, or Web-search behavior. A channel that works with `curl` can still fail in a normal Grok Build conversation, lose native Web tools, call `List` instead of `list_dir`, emit an empty `finish_reason`, dump chain-of-thought after the answer, or receive the wrong login credential. Relayed grok-4.5/4.6 endpoints have the same problem when the relay rewrites the API as Chat Completions or Responses.
...
- **Not a team gateway.** hellogrok is a single-user local proxy: no multi-tenancy, no request auditing, no rate governance. Its resilience semantics optimize for "one unattended conversation survives a relay blip", not for organization-wide failure observability.
- **Not a strict-SLA client.** The `balanced` tier deliberately delays deterministic errors by up to the absorb window while it polls for relay-side recovery. When you are actively debugging a channel, keep the default `off` tier so rejections surface immediately.
- **Not needed for first-party endpoints.** A direct, compliant official API gains nothing from the compatibility layer; connect to it without the proxy.
...
- Keeps the provider's configured upstream protocol, URL path, model, credential, reasoning, and tool semantics at the provider boundary. Ordinary Chat and Messages channels stay on Grok Build's first-party mapper and sampler. Responses projection is used only when hosted search requires it (`supports_backend_search = true` or Grok Build's WebSearchClient hop).
...
- When `supports_backend_search = false`, keeps Grok Build on the configured native consumer so its client `web_search` uses `[models].web_search`, `GROK_WEB_SEARCH_MODEL`, or the authenticated official fallback. Omission preserves Grok Build catalog behavior for every provider, DeepSeek included: after DeepSeek retired its hosted web search (the Responses API now ignores `web_search` tools), an omitted field uses client search instead. An explicit false opts out unless that custom route is selected as the default search model, and an explicit true is still honored as declared.
...
- Converts provider-private `keepalive`, `keep-alive`, `keep_alive`, `heartbeat`, and `ping` frames into standard SSE comments before they reach Grok Build, without consuming Responses sequence numbers, and closes each upstream stream as soon as its protocol terminal event arrives. Terminal detection is per protocol, not per channel or model: Chat Completions `finish_reason`, Messages `stop_reason`/`message_stop`, and Responses `status` completed/failed/incomplete or collected output items. A relay that then omits `[DONE]`, `message_stop`, or `response.completed` and closes cleanly gets that trailer synthesized instead of a `proxy_stream_error` that would make Grok Build retry a completed turn.
...
- Temporarily materializes `inference_idle_timeout_secs = 900` for proxied channels that do not configure one (restored byte-for-byte on stop), so relays that emit heartbeats but no content while queued — stalls of 3+ minutes have been observed — no longer hit Grok Build's 600-second default content deadline; explicit per-model or global `[models]` values remain user-owned. Every streaming path also runs a downstream content watchdog that mirrors Grok Build's per-protocol content classification (text, reasoning, tool-call deltas, and terminal signals count; heartbeats, empty deltas, usage-only chunks, and lifecycle bookkeeping events do not) and fires one 30-second margin ahead of the client's own content timer. On fire it closes the upstream and emits a retryable `proxy_stream_error`, so a genuinely stalled stream re-enters Grok Build's native 15-attempt retry budget instead of dying on the client's non-retryable `IdleTimeout`. Explicit timeouts below 60 seconds disable the watchdog to respect fast-fail choices.
- Absorbs transient upstream soft failures before Grok Build ever sees them. While the upstream response headers have not been sent, a retryable failure (busy/overloaded `503`, `429`, retryable `5xx`, or a response-header timeout) is retried inside the proxy with exponential backoff (2s to 30s, honoring an upstream `Retry-After` up to 60s) for up to 90 seconds by default. A Responses SSE whose only terminal so far is a retryable `response.failed` (rate limit, concurrency, overload) is withheld the same way — response headers and early frames stay buffered (up to 32 frames) so the retry does not become a second stream — and replayed inside the same window; `absorb_retry_max_secs = 0` streams that failed event instead. An upstream `408` is treated as what it actually is between two servers — an edge/gateway timeout page, semantically a `504` — so it enters the same window and, if the window is exhausted, passes through remapped to a retryable `504` instead of Grok Build's terminal `408` classification. Cloudflare origin-TLS failures (`525`, `526`) are absorbed the same way even though Grok Build classifies them terminal: a relay origin restart or certificate rotation routinely clears inside the window, and a request that failed the edge-origin TLS handshake never reached the origin application, so the replay is side-effect free; when the window is exhausted they pass through with the terminal disposition Grok Build expects. Grok Build's own 15-attempt retry budget stays untouched during the window: it only sees a failure when the window is exhausted, and even then the failure passes through as retryable, so the client keeps its full native budget. Transport errors are not absorbed because Grok Build's first-retry HTTP/1.1 client rebuild handles those better. Streaming failures after headers are sent are unaffected. `absorb_retry_max_secs = 0` disables the window per channel.
- Never shortens Grok Build's native retry budget for soft failures. Only deterministic errors whose retries cannot succeed (authentication, permission, billing, quota, invalid request/model) receive `X-Should-Retry: false`, so Grok Build shows the provider explanation instead of hiding it behind repeated retries. The default passthrough disposition mirrors Grok Build's edge-client policy — `429` and every `5xx` except the origin-TLS codes are retryable — so a transient Cloudflare edge page (`520`–`524`, `529`, `530`) keeps the client's native retry instead of being vetoed by a proxy-stamped header. When a retryable busy/overloaded `503` passes through without an upstream `Retry-After`, the proxy adds a 30-second one so Grok Build paces its retries.
- Absorbs soft failures for every channel, always: retryable `5xx`, `429`, upstream `408` edge-timeout pages, Cloudflare origin-TLS `525`/`526`, and response-header timeouts are retried inside the window because they clear on their own and Grok Build's retry of them only burns time. The global `[models]` `error_resilience` setting only controls deterministic 4xx failures (authentication, permission, invalid request/model): by default (`off`) they pass through immediately with the provider's explanation and `X-Should-Retry: false`, because retrying them cannot succeed; the opt-in `balanced` value additionally absorbs them inside the window with a fixed 30s-then-60s wait sequence, because relays routinely recover from those on their own — token rotation, permission fixes, provider config reloads, brief deploys — and an unattended turn survives a multi-minute outage instead of dying on the first response. A reasoning-history rejection is never absorbed: it reports a foreign conversation state the user must see, and hiding it would only delay the recovery.
...
- Recognizes relays that answer a failure with HTTP 200 plus an error envelope, a common newapi/sub2api defect. A transient envelope (rate limit, overloaded, timeout) enters the absorb window and is retried inside the proxy; a deterministic envelope (authentication, billing, invalid request) passes through with the provider's explanation instead of an opaque envelope-validation rejection. A Responses terminal body that carries its own error member keeps its native failed-response path untouched.
- Recovers from upstream vision rejections on text-only channels: when a channel answers `400` with a `vision` error code or a `not multimodal` / `does not support image` message, hellogrok replaces every image content part (`image_url`, `input_image`, `image`, including images inside tool results such as `read_file` of a picture) with a text placeholder and retries the request once, so a text-only upstream no longer ends the turn on `vision_not_supported`. Without retained per-channel state, a later image-bearing request to the same channel pays one rejected attempt again before the strip retry succeeds.
- Treats a Cloudflare shield challenge in front of a relay as transient: a `cf-mitigated: challenge` header, or a Cloudflare-served challenge page, is absorbed inside the retry window, and a challenge that outlasts the window passes through as a retryable `503` because a bare `403` would classify terminal in Grok Build and kill the turn on a self-clearing shield. A genuine origin `403` without Cloudflare markers is untouched.
- Synthesizes missing Responses envelope bookkeeping (`id`, `object`, `status`) when a relay omits it on an otherwise complete terminal body, instead of rejecting the repairable omission; present-but-wrong values and a missing or malformed `output` remain rejections. A probe that never reports an outcome — the caller disconnected mid-probe, or the probe's response headers timed out — is presumed lost after a 2-minute lease, and a later request may probe again instead of leaving the channel latched open.
- Logs the model declared by the raw upstream response before normalization, including terminal-frame precedence, case-insensitive mismatch detection, and conflicting declarations, without changing routing or response data. Benign mismatches log once per channel/protocol/configured/upstream pair; conflicts and invalid declarations always log. When Grok Build sends an official catalog name such as `grok-4.6` on a non-xAI custom channel, the proxy logs a separate warning with body size, tool count, and session presence to aid `/resume` diagnosis.
- Rejects protocol-shape mismatches explicitly: an SSE response to a non-streaming request, or a streaming response to Grok Build's fixed non-streaming WebSearchClient request, returns a non-retryable `502` naming the mismatch instead of forwarding an undecodable body.
- Logs upstream failures as structured summaries (`type`, `code`, `message`) for HTTP errors and Responses `response.failed`/`error` events, with bearer tokens, key assignments, and `sk-` values redacted and long messages truncated, so diagnostics never persist credentials. A stream that ends because Grok Build disconnected is logged as aborted by the client and does not emit `proxy_stream_error`.
...
- Normalizes missing local tool-call IDs before delivery in native Chat and Messages responses. Responses streams retain output-item and function-call identities across events; a provider that reuses one upstream ID across output slots or Chat tool calls receives a fresh unique ID for the colliding slot instead of a dropped stream, while genuine same-slot conflicts are still rejected. The same tolerance covers Chat Completions `tool_call` IDs reused across parallel calls, in streaming and non-streaming responses alike; Chat JSON-to-SSE fallback assigns independent indexes to parallel tool calls. Existing Chat history is repaired only when one missing call ID has one unambiguous matching result; ambiguous histories remain errors.
- Reassembles native Chat tool-call SSE before Grok Build's last-write-wins accumulator sees it: empty later `"name"` fields do not erase a known name, and argument fragments are not rewritten until the JSON is complete. The three frame-rewriting stream rectifiers (Chat, Messages, Responses) share a common `streamFrameRectifier` contract so new protocols plug into the same pipeline. Empty or vendor `finish_reason` values are dropped or mapped onto Grok Build's Chat enum (`stop`, `length`, `tool_calls`, `content_filter`, `function_call`).
- Repairs tool-call streams whose first delta was lost in transit. When a Chat, Messages, or Responses stream arrives with an empty function name and arguments truncated at the head (the opening `{"` missing), hellogrok restores the prefix whenever the repaired payload parses as one complete JSON object, infers the name from the argument key set against the tools declared on that request (the shared `command`+`description` shape resolves deterministically to `run_terminal_command`), and applies the same repair to non-streaming responses and to persisted history replayed in later requests. Chat tool frames are held until the stream terminal so argument fragments a relay emits after `finish_reason` are merged instead of dropped; Messages `tool_use` blocks and Responses `function_call` items are held between their start and stop frames for the same reason, with the terminal frame's complete item as a second source for name and arguments. Fragments discarded after an early flush are logged as `late-tool-deltas-discarded`.
- Lifts Chat dialects Grok Build does not deserialize: array `content`, `thinking`/`reasoning`/`reasoning_details`, Gemini `functionCall`, and `<think>`…`</think>` in the answer text. Think spans are stripped wherever they appear in the stream, not only at its head: thinking models emit several CoT phases per turn and relays sometimes misroute a reasoning tail into `content` with only the closing tag, so second-phase spans, unbalanced `</think>` tails, stray closing tags, and tags split across stream deltas are classified as reasoning or removed instead of leaking into the visible reply. Tags quoted with backticks or double quotes are prose about the tags, not stream delimiters: reasoning that discusses the tags keeps its full span in the reasoning channel instead of terminating early, and quoted tags inside visible text reach the client intact. Reasoning is forwarded only as a prefix sibling; later thought after visible text is dropped so the TUI does not open a second Thought under the reply. Streaming `reasoning_content` deltas are concatenated as-is so leading BPE spaces are not trimmed per frame.
...
- Streaming Chat requests keep Grok Build's `stream_options.include_usage=true` so terminal usage can drive auto-compaction. GLM Chat channels with tools also receive `tool_stream=true` when the field is omitted.
...
- Presents every upstream request as Grok Build: an incoming `grok-shell` User-Agent and `X-Grok-Client-Identifier` are kept; otherwise they are synthesized from the installed `grok --version` or a version-less `grok-shell` fallback. hellogrok's own product string is never forwarded. OpenCode still prefers an explicitly configured User-Agent.
...
- Upstream tool lists keep Grok Build `client_name` values (`list_dir`, `read_file`, `run_terminal_command`, `spawn_subagent`, …). Claude/Codex aliases (`LS`, `Read`, `Bash`, `Task`, `Grep`, `Write`, `ToolSearch`, …) are rewritten on inbound calls rather than cloned into every request's `tools` array.
- Inbound calls are rewritten to the declared `client_name` and parameter keys. Top-level Chat `name`/`arguments`, legacy `function_call`, object-valued arguments, empty names recovered from argument shape (including arguments truncated by a lost first stream delta), and common XML/JSON-in-content calls are normalized before Grok Build dispatches. Parameter aliases are normalized per declared schema (`target_path` becomes `target_file`, `target_directory`, or `file_path`), and a streamed call that accumulates no argument fragments at all is discarded before dispatch with an `empty-args-discarded(name=...)` log note when the declared tool requires properties, because Grok Build would otherwise report a guaranteed `missing field` parse failure for the empty arguments object. An unnamed call that shape inference cannot resolve is routed to a self-healing carrier instead of reaching Grok Build with an empty name: parseable arguments go to `run_terminal_command` (`command` carries the raw text) and corrupt arguments go to `read_file` (`target_file` carries the raw text), logged as `unresolved-name-routed(name=...)`, so the failure returns through a `tool_result` and the model regenerates the call; when neither carrier is declared the call is left untouched.
...
- With `supports_backend_search = true`, every supported format uses the current channel's own hosted search: Responses stays Responses, Messages stays Messages with `web_search_20250305`, and Chat uses its configured search dialect or protocol bridge. First-party DeepSeek follows the configured native protocol now that its Responses API is GA; Chat hosted search defaults to a Responses bridge because DeepSeek Chat documents function tools only. Explicit `chat_search_dialect = "messages"` remains available when a Messages bridge is required. Note that DeepSeek has retired its hosted web search — its Responses API now ignores `web_search` tools — so the flag is only useful there for relayed endpoints that still implement a search extension.
- For a channel marked `supports_backend_search = true`, the Grok Build side receives canonical Responses search events when the upstream actually executes search, including completed `web_search_call` items and any verified sources, citations, and usage the provider returned. The flag is a routing declaration, not a capability probe. Every Responses search asks for complete `web_search_call.action.sources` metadata while preserving caller entries; Volcengine Ark (`*.volces.com`) treats `include` as a closed enum and rejects unknown values with `InvalidParameter`, so its hosted search omits the hint and source display falls back to its response annotations; declaration fields Ark's closed schema rejects (unknown tool fields such as `filters`, and the `allowed_tools` selector) are stripped, with a pure hosted-search selection collapsed to the equivalent `required`. Any other upstream that proves its schema closed with a 400 (include or tool-declaration rejection) triggers one in-request rewrite-and-replay — strip the include or sanitize the declaration — without retaining per-host state; a rejected request never executed a search, so the replay is side-effect free. Citation entries missing index fields required by Grok Build's parser are filled with 0 on every route. DeepSeek currently accepts but silently ignores that standard `include` hint, so hellogrok consumes any URLs already present in its native search action but cannot recover omitted results. If DeepSeek implements the hint later, the same request begins receiving sources without a hellogrok update. Messages search-result/citation blocks, provider-level citations, and Chat annotations/search-result containers are normalized into the same canonical source fields.
...
- Cross-checks every provider usage report against the physical request before it reaches Grok Build's context meter. Per conversation (channel plus the client's session identity), the guard learns a bytes-per-token ratio from the first coherent report — the tokenizable footprint of the request body, encoded size minus embedded base64 data URIs, divided by the reported prompt tokens — and drops any later report whose prompt count exceeds 1.25× what the current body can hold. Such reports double-count the cached prefix or carry session-cumulative totals, and would otherwise move the meter in one step and fire auto-compaction far above the configured threshold. A dropped report leaves Grok Build on its own byte-based estimate until the provider reports coherently again; a body that shrinks by more than a quarter (compaction, rewind, resume) relearns the baseline, and conversations without a stable session identity skip the check.
- Rectifies provider usage measurements before they reach Grok Build's token ledger and auto-compaction meter. Stringly-typed counts (`"prompt_tokens": "12345"`), whole floats, and `json.Number` values are normalized to integers; invalid optional decorations (null detail values, non-map detail containers, fractional `cost_in_usd_ticks`) are removed without poisoning the core measurement; unrecoverable core counts (negative, fractional, overflowing, non-numeric) are deleted so the downstream validator drops the measurement instead of trusting corrupt data. Repairs apply across Chat Completions, Responses, and Messages, in streaming and non-streaming responses, and in protocol-translation paths, and are logged as `usage rectified` proxy notes.
...
| Any channel with `supports_backend_search = false` | Grok Build uses client `web_search`: first `[models].web_search` or `GROK_WEB_SEARCH_MODEL`, otherwise its authenticated official fallback. A selected custom search route is temporarily overridden to true. An omitted field preserves Grok Build catalog behavior for every provider, including the first-party DeepSeek endpoint. |
...
| `reasoning_effort` | No | Grok Build selection | Selected or default Grok Build reasoning level. This value is user- or catalog-owned and is never written or reordered by hellogrok; it can be configured without also defining a reasoning menu. |
| `reasoning_efforts` | No | Model catalog | Reasoning levels exposed by Grok Build for the model. String arrays and custom object menus remain unchanged; hellogrok translates protocol field shapes when required but does not reinterpret the selected level. |
| `supports_reasoning_effort` | No | Model catalog/menu | Explicit reasoning-selector capability. hellogrok reads it only to preserve protocol semantics and never inserts or changes it. |
| `supports_backend_search` | No | Catalog/provider default | When true, uses this channel's own hosted search and exposes canonical Responses search events to Grok Build for all three upstream formats. When false, Grok Build uses its configured or authenticated client-search route unless this custom model is selected by `[models].web_search` or `GROK_WEB_SEARCH_MODEL`, which temporarily overrides it to true. Every model, including the first-party DeepSeek endpoint, remains owned by explicit configuration or Grok Build's model catalog. |
...
| `inference_idle_timeout_secs` | No | Managed `900`, else Grok Build/provider policy | Maximum idle gap while waiting for upstream headers or body data. Grok Build additionally applies the same value as its per-chunk content-progress deadline: text, reasoning, tool-call deltas, and terminal signals count as progress, while heartbeats and empty deltas do not. A model value overrides the global `[models]` value; when neither exists, hellogrok temporarily materializes `900` for every proxied channel (restored byte-for-byte on stop) and mirrors it into the live route so proxy timers stay consistent. Every streaming path also runs a downstream content watchdog 30 seconds ahead of that deadline — a stalled stream becomes a retryable `proxy_stream_error` instead of the client's non-retryable `IdleTimeout`. Explicit values below 60 seconds disable the watchdog to respect fast-fail choices. |
| `absorb_retry_max_secs` | No | `90` | Total wait window for proxy-side retries of transient upstream soft failures (busy/overloaded `503`, `429`, retryable `5xx`, response-header timeouts, upstream `408` edge-timeout pages, and Cloudflare origin-TLS `525`/`526`). Grok Build's retry budget stays untouched while the window lasts, and once the window is exhausted Grok Build's own retry indicator shows the error reason (`<headline> | Retrying (N/M)`) while its native budget continues. An explicit `0` disables the absorb layer for the channel. |
| `absorb_retry_backoff_cap_secs` | No | `30` | Cap for each absorb-layer backoff step. An upstream `Retry-After` on the error response still paces the waits, capped at 60 seconds. |
| `error_resilience` | No | `off` | Global `[models]` setting (not per channel). Controls deterministic 4xx failures only (soft-failure absorption is always on): `off` passes authentication, permission, and invalid request/model rejections straight through with the provider's explanation; `balanced` additionally absorbs them with fixed 30s-then-60s waits so unattended turns survive relay-side token rotation, permission fixes, and brief deploys. |
| `dead_channel_fail_fast` | No | `false` | Opt-in fast-fail for a truly unreachable channel: after consecutive dial-level failures (connection refused, DNS, TLS handshake) the proxy answers a non-retryable `503 proxy_circuit_open` instead of burning Grok Build's retry budget. Soft failures never count. |
| `dead_channel_fail_threshold` | No | `6` | Consecutive dial-level failures required to open the opt-in dead-channel breaker. |
...
DeepSeek compatibility is enabled by the exact first-party host `api.deepseek.com`, independently of model ID. Rolling aliases and future models therefore inherit authentication, endpoints, protocol-level request normalization, queue keep-alives, usage handling, and SSE framing without a hellogrok release. Native same-protocol requests retain unknown fields; cross-protocol bridges map the fields Grok Build currently sends that the target protocol can express. Reasoning-effort strings remain provider-owned and pass through without a hellogrok model-level mapping table. As announced on 2026-09-10, the current production IDs are `deepseek-flash` (the V4.1 Flash generation with native vision; the retired `deepseek-v4-flash` and `deepseek-v4-flash-vision-exp` names are still accepted and routed to it) and `deepseek-v4-pro`; these names are examples, not a hellogrok allowlist. A relay that merely reuses a DeepSeek model name receives no first-party assumptions.
...
The retired `deepseek-v4-flash` name is still accepted by DeepSeek and routed to V4.1 Flash, so an existing configuration keeps working unchanged.

DeepSeek has retired its hosted Web Search: the Responses API now ignores `web_search` tools, so an omitted `supports_backend_search` uses Grok Build client search (via `[models].web_search`, `GROK_WEB_SEARCH_MODEL`, or the authenticated official fallback), matching every other provider. An explicit `true` is still honored as declared but the first-party endpoint silently ignores the search extension. Responses and Messages hosted-search requests stay on their configured native APIs. Chat defaults to a Responses bridge because DeepSeek Chat currently documents only function tools; set `chat_search_dialect = "messages"` only when a Messages bridge is intentionally required. Native Responses and Chat authenticate with Bearer. Messages uses the documented `https://api.deepseek.com/anthropic` SDK base URL (the effective endpoint is `/anthropic/v1/messages`) and `X-Api-Key`; the `[1m]` suffix is accepted only as DeepSeek's documented Anthropic integration alias and must not be used with Responses.
...
Grok Build owns the model's reasoning menu and selected/default level. hellogrok never adds, migrates, reorders, or replaces `reasoning_effort`, `reasoning_efforts`, or `supports_reasoning_effort`; user-defined string arrays, object labels, order, and defaults remain byte-for-byte in place. Users may configure only `reasoning_effort`, only a menu, both, or neither. For example, a user who wants only DeepSeek's distinct native choices can configure `reasoning_efforts = ["none", "low", "high", "max"]`; a user who needs only a fixed selection can set `reasoning_effort` alone. Omitting all three fields leaves Grok Build model-catalog and provider defaults intact.

hellogrok does not reinterpret provider effort levels. A Responses-to-Messages bridge mirrors Grok Build's native Messages serializer by omitting `none` and `minimal`; every other non-empty value is copied unchanged. On the first-party DeepSeek endpoint, an explicit `none`, including a standalone `reasoning_effort = "none"`, becomes that protocol's native thinking-off switch so the request does not fall through to DeepSeek's enabled-by-default mode. When no reasoning fields are configured, hellogrok adds no thinking switch and preserves that default. DeepSeek's current documentation describes the following server-side behavior:

| Requested level | DeepSeek effective level |
|-----------------|--------------------------|
| `low` | `low` |
| `medium` | `high` |
| `high` | `high` |
...
This table is DeepSeek service behavior, not a transformation performed by hellogrok. A menu limited to `none`, `low`, `high`, and `max` avoids duplicate choices that resolve to the same effective level. Any other non-empty effort remains unchanged on the provider request, so DeepSeek's current or future validation is authoritative. Third-party relays receive no DeepSeek-specific reasoning adaptation apart from ordinary cross-protocol field conversion.

### Native capability mapping

| DeepSeek surface | hellogrok behavior in Grok Build |
|------------------|----------------------------------|
| Responses | Preserves `instructions`, developer messages, reasoning, native `text.format` JSON Schema, function tools, and native Web Search blocks when the provider actually returns them. It sends the standard source `include` hint without inventing missing results; source display is limited to URLs DeepSeek actually returns. Provider `action.queries` arrays remain intact while each call gains the singular `action.query` field Grok Build displays. It handles `response.completed`, `response.incomplete`, and `response.failed` as terminal events. |
...
| Anthropic Messages | Uses `X-Api-Key`, preserves thinking/tool history, effort control, function tools, and native server Web Search blocks when present. When a reasoning selector is explicitly configured, an omitted Grok Build `None` selection becomes `thinking.type=disabled` instead of falling through to DeepSeek's default `high`; only the supported `output_config.effort` is sent, explicit Responses `user` maps to `metadata.user_id`, and the documented `deepseek-v4-pro[1m]` alias remains on the Messages wire. |
| Queue and usage | Accepts non-streaming empty-line keep-alives and streaming `: keep-alive` comments. Real terminal usage is preserved so Grok Build can drive context accounting and auto-compaction. Measurements larger than the known `context_window` stay unknown. |
...
hellogrok preserves the upstream status and error body. Structured rate-limit, timeout, overload, and temporary-unavailable errors remain retryable, and the proxy first retries them inside its own absorb window (`UP absorb` log lines) before the client ever sees them; once the window is exhausted they pass through retryable and Grok Build's retry indicator shows the reason (`Service unavailable | Retrying (N/M)`, `Request timed out (504) | Retrying (N/M)`), so the failure stays visible while the native budget continues. Deterministic errors (authentication, permission, billing, insufficient-balance/quota, invalid request/model) receive `X-Should-Retry: false` and surface immediately by default; the opt-in `[models]` `error_resilience = "balanced"` setting also retries them inside the absorb window first with 30s/60s waits, because relays routinely recover on their own (token rotation, permission fixes, config reloads), and the provider explanation still passes through unchanged if the window is exhausted — with one explicit trade-off: an error that outlasted a long window is the last attempt's snapshot, not a live one, so hellogrok stamps `X-Hellogrok-Absorb-Delay` on the response and prefixes structured error messages with a `[hellogrok: upstream stayed failing for ...]` note. The delay you see is real; the error is as fresh as the last retry. If a relay returns only a generic `503`, hellogrok cannot infer that the account is out of balance; the relay must return a billing code or message.
...
### `Request timed out (408)` from a relay

An upstream 408 between two servers is never the RFC "client took too long" case — there is no idle client between hellogrok and the relay. Cloudflare and relay gateways emit 408 for edge-side timeouts while the origin is queued or restarting. hellogrok therefore treats 408 as a 504: it is retried inside the absorb window, and a 408 that outlasts the window passes through as a retryable 504 so Grok Build keeps its native retry budget instead of failing the turn on its terminal 408 classification. A recurring 408 that never clears inside the window means the relay is saturated for minutes at a time; switch channels or ask the relay operator about queue capacity.

### `Server error (525)` from a relay

HTTP 525 means the Cloudflare edge in front of the relay could not complete a TLS handshake with the relay's own origin; 526 means the origin certificate is invalid. The request never reached the relay's application. Grok Build classifies both statuses as terminal — a broken origin certificate never clears on its own — and shows its generic server-error copy, so the turn fails on the first occurrence unless the failure clears inside hellogrok's absorb window, which retries 525/526 inside the proxy by default. A single isolated 525 that disappears on resend is the relay's origin restarting or rotating certificates; check the relay's status page. If a channel reports 525 repeatedly, its origin certificate or TLS termination is misconfigured and only the relay operator can fix it: switch channels with `/model`, or raise `absorb_retry_max_secs` when the relay's rotations routinely take longer than the default window.

### Stuck on “retrying” forever

Grok Build retries retryable statuses (429, 5xx) up to 15 times, roughly 5.5 minutes per turn. Transient provider overload (a busy `503`) is hidden first by hellogrok's absorb window and then paced by a synthesized `Retry-After`, so a short outage usually never reaches the UI at all. A turn that stalls with heartbeats but no content first waits out the channel's idle deadline — 900 seconds when hellogrok manages it — before the retry phase begins; a stream that never produces content past the watchdog margin is converted to a retryable `proxy_stream_error` (logged as `SSE stalled: no content reached Grok Build for …`) instead of dying on the client's non-retryable `IdleTimeout`. When a relay's origin stays down for longer than the absorb window, the client keeps its full retry budget and the UI stays in the retrying phase — the expected native behavior.
...
- The channel is unreachable at the network level, not merely busy. Check the relay's own status and your network path; the `UP breaker` lines in the proxy log record the dial-level failures.
- If connectivity is back, wait about 5 minutes and send again; the probe closes the breaker automatically without restarting the proxy or reselecting the model.
- If the error repeats, switch to another channel with `/model` until the relay is fixed.
...
Grok Build dispatches by exact `client_name` (`list_dir`, not `List`). A third-party or relayed model may emit Claude/Codex names, a top-level Chat `name`, an empty `function.name` on a later SSE chunk, XML in the message body, or a direct MCP name. A relay can also lose a tool call's first streamed delta, leaving an empty name and arguments missing their opening `{"`. Current hellogrok reassembles Chat tool streams, holds Messages `tool_use` blocks and Responses `function_call` items until their stop frames, repairs truncated argument prefixes, recovers empty names from argument shape, rewrites those calls to the tools declared on that request, and repairs history `name` fields on live responses and replayed history alike. A call that still has no name after shape recovery is routed to a self-healing carrier (`run_terminal_command` for parseable arguments, `read_file` for corrupt arguments, logged as `unresolved-name-routed`) so Grok Build parses it and the raw arguments return to the model through a `tool_result` instead of dying as a silent terminal `NotFound`.
...
For ordinary channels, hellogrok waits at most 601 seconds for upstream response headers and at most 601 seconds between reads from any response body, including non-streaming JSON and error bodies. This stays one second beyond Grok Build's 600-second shell default, so Grok Build remains the owner of its native timeout classification. First-party `api.deepseek.com` routes use 660 seconds because DeepSeek can queue a request for up to ten minutes. A non-streaming queue newline or streaming heartbeat counts as data and renews that idle window. A header timeout is a soft failure: the proxy first retries it inside the absorb window, then returns retryable `504` when the window is exhausted. While reading a non-streaming body the timeout returns retryable `504` immediately; after a `200` stream has begun, hellogrok emits the receiving protocol's `proxy_stream_error` and closes the upstream. There is no total request timeout, so an active long-running response is not terminated. Byte activity and content progress are tracked separately: heartbeats renew the upstream byte window above, while Grok Build's content-progress timer only counts real content (text, reasoning, tool-call deltas, terminal signals). Every streaming path therefore also runs a downstream content watchdog, mirrored on the channel's idle deadline minus 30 seconds; when a stream carries bytes but no content past that point, the proxy closes the upstream and emits a retryable `proxy_stream_error` so the turn enters Grok Build's native retry budget. The `response_model` log line shows the upstream-declared and configured models; `mismatch=true` identifies a relay that silently substituted a model, while `conflict=true` means different response frames declared different models.
...
Complete input/output usage gives an exact baseline for ordinary DeepSeek requests. Server-side Web Search may return cumulative billing totals instead of the final live prompt. When a prompt or output count exceeds the known `context_window`, hellogrok discards that measurement (`usage: null`) rather than asking Grok Build to compact on an impossible live context. A trustworthy in-window pair, including a provider `context_details` extension that itself fits the window, still updates the baseline. The same guard applies to non-DeepSeek channels. A provider that omits usage can only retain the prior baseline and grow approximately. Explicit `context_window` remains required for this check.

The mirror failure is a meter that jumps far above the configured threshold in one step. Grok Build writes the reported `total_tokens` straight into its context meter, so a single relay response that double-counts the cached prefix — or reports session-cumulative prompt tokens — can move a session from half full to past the auto-compact threshold although the conversation never grew. hellogrok therefore cross-checks every usage report against the tokenizable request body per conversation and drops reports whose prompt count overshoots the body-derived expectation by more than 25%, leaving Grok Build on its byte-based estimate until the provider reports coherently again. When compaction still fires at a surprising percentage, compare the proxy log's `usage suppressed` line with the request body size: a suppressed report is a provider misreport, not a threshold misconfiguration.
...
### Thought appears under the finished answer

Grok Build closes the current Thought on the first visible reply. Official grok streams reasoning as a prefix sibling; a later `reasoning_content` / `thinking` delta opens a second Thought under the answer. hellogrok drops post-answer thought on Chat, Messages, and Responses and peels protocol self-talk such as `reply only:` / `任务已全部完成` from prefix CoT. Restart the proxy after upgrading and start a new session; stored bubbles are not rewritten.

### Reasoning text appears inside the reply

Thinking models wrap CoT in `<think>`…`</think>`, and relays sometimes dump reasoning into `content` mid-turn or with only the closing tag. hellogrok strips think spans wherever they appear in the stream: second-phase spans, unbalanced `</think>` tails, and stray or delta-split closing tags are routed to the reasoning channel or removed instead of leaking into the visible reply. Tags quoted with backticks or double quotes are treated as prose: reasoning that mentions the tags does not end its span early, and quoted tags in the reply are preserved. Reasoning that arrives after visible reply text is dropped by design, because Grok Build renders reasoning only as a prefix Thought. Stored bubbles are not rewritten; restart the proxy after upgrading and start a new session.

### A tool call fails with `missing field` argument errors

Grok Build validates tool arguments against the declared schema and echoes the original arguments on failure. Third-party models sometimes emit aliased parameter names (`target_path` instead of `target_file`) or, rarely, no argument fragments at all. hellogrok normalizes known aliases per declared schema and discards argument-less calls to tools with required properties (logged as `empty-args-discarded`), because an empty arguments object can never satisfy them. If the error still appears, the echoed arguments show exactly what the model sent; the failed call is fed back to the model, which usually retries with corrected arguments on the next turn.

### Image input is rejected with `vision_not_supported`

Text-only upstreams reject requests that carry image content parts, including images a `read_file` of a picture placed into a tool result. hellogrok recognizes the rejection (`400` with a `vision` error code or a `not multimodal` / `does not support image` message), replaces every image part with a text placeholder, and retries the request once, so the turn continues without the visual payload and the placeholder tells the model the image was omitted. Channels are not remembered as text-only, so each image-bearing request pays one rejected attempt before the strip retry succeeds.

### `read_file` reports `exceeds maximum allowed tokens`

This is Grok Build's own per-call cap, not a hellogrok error: one `read_file` window is limited to 25,000 estimated tokens (1,000 lines by default), and a dense file over the cap fails wholesale instead of being truncated. Read such files with `offset`/`limit` ranges, or locate the region with `grep` first. The red `Read … failed` marker is informational; the turn continues with a ranged retry.

### The model keeps repeating a wrong fact from its previous thought

Plaintext CoT replay can lock a later user turn onto an earlier generation error. On Chat, hellogrok keeps the model's own `reasoning_content` inside the current tool loop and strips it across user turns, except DeepSeek and MiMo, whose gateways require the field. Encrypted or signed reasoning is never stripped. Start a new session after upgrading; old assistant text already in history is unchanged.

### `List .` in the tool header

That is Grok Build's TUI: listing the workspace root is shown as `.`, including for official grok. The call still used the real directory (`.` or the absolute cwd). It is not an empty path and is not rewritten by hellogrok.
...
- Chat history does not replay plaintext CoT across user turns except on DeepSeek and MiMo. Intra-turn tool-loop reasoning is kept. This matches Grok Build's native Chat mapper plus vendor gateway contracts; it is not a second protocol stack.
- Responses-to-Messages/Chat conversation conversion is enabled only for capability-enabled channels (explicit `supports_backend_search = true`, a selected default search model, or a hosted-search request resolved from Grok Build's remote model catalog), plus Grok Build's fixed non-streaming WebSearchClient request. Other cross-protocol requests are rejected.
