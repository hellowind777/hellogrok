...
[![Version](https://img.shields.io/badge/version-0.1.39-2f6feb.svg)](./internal/appinfo/appinfo.go)
...
- Temporarily materializes `inference_idle_timeout_secs = 1800` for proxied channels that do not configure one (restored byte-for-byte on stop; first-party `api.deepseek.com` routes are excluded so their remote metadata stays authoritative), so relays that emit heartbeats but no content while queued — stalls of 3+ minutes have been observed — no longer hit Grok Build's 600-second default content deadline; explicit per-model or global `[models]` values remain user-owned. Every streaming path (including the DeepSeek route) also runs a downstream content watchdog that mirrors Grok Build's per-protocol content classification (text, reasoning, tool-call deltas, and terminal signals count; heartbeats, empty deltas, usage-only chunks, and lifecycle bookkeeping events do not) and fires one 30-second margin ahead of the client's own content timer. On fire it closes the upstream and emits a retryable `proxy_stream_error`, so a genuinely stalled stream re-enters Grok Build's native 15-attempt retry budget instead of dying on the client's non-retryable `IdleTimeout`. Explicit timeouts below 60 seconds disable the watchdog to respect fast-fail choices.
