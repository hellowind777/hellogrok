...
| `absorb_retry_max_secs` | No | `90` | Total wait window for proxy-side retries of transient upstream soft failures (busy/overloaded `503`, `429`, retryable `5xx`, response-header timeouts, and Cloudflare origin-TLS `525`/`526`). Grok Build's retry budget stays untouched while the window lasts. An explicit `0` disables the absorb layer for the channel. |
