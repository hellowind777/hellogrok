...
- With `supports_backend_search = true`, every supported format uses the current channel's own hosted search: Responses passes through, Messages receives `web_search_20250305`, and Chat uses its configured search dialect or protocol bridge. First-party DeepSeek is the source-preservation exception: when hosted search is eligible in the current request, it defaults to the official Messages API so structured result URLs survive. A tool choice that disables or excludes search keeps the configured protocol. Set `chat_search_dialect = "responses"` only to opt eligible search requests back into the source-limited Responses search surface.
...
hellogrok never creates, selects, or replaces `[models].web_search`, and startup sends no search-capability probes. A Messages channel marked enabled must actually support `web_search_20250305`. Chat defaults to `web_search_options`; official xAI Chat bridges to Responses, while first-party `api.deepseek.com` hosted search defaults to Messages for every configured backend because its Responses API does not expose requestable source details. Set `chat_search_dialect` to `web_search_options`, `search_parameters`, `messages`, or `responses` when a relay needs an explicit strategy or DeepSeek must opt out of that default. The temporary flag is a routing declaration, not proof of provider capability. A real upstream HTTP rejection is preserved; if the fixed WebSearchClient request is silently answered without a completed search, hellogrok returns a non-retryable `502` stating that the selected search model did not complete backend `web_search`.
...
| `api_backend` | No | Model catalog, then `chat_completions` | Native upstream API format: `responses`, `messages`, or `chat_completions`. A capable non-Responses channel is temporarily projected as Responses only to Grok Build. First-party DeepSeek requests in which hosted search is eligible default to Messages to retain source URLs; requests whose tool choice excludes search, other traffic, and the restored config keep the selected format. |
| `chat_search_dialect` | No | Host-based | Hosted-search strategy override: `web_search_options`, `search_parameters`, `messages`, or `responses`. Defaults are `messages` for first-party `api.deepseek.com`, `responses` for official xAI Chat, and `web_search_options` for other Chat routes. |
...
Leave `supports_backend_search` unset to use DeepSeek's native hosted Web Search, or set it explicitly to `false` to use Grok Build client search instead. When hosted search can run, first-party DeepSeek defaults to the official Messages API for Responses, Messages, Chat, and unconfigured routes because that surface returns structured `web_search_tool_result` URLs. If `tool_choice` disables search or selects another function, the request stays on its configured API. This search-only protocol choice is host-based, not model-ID-based. Set `chat_search_dialect = "responses"` only when Responses-specific search/tool behavior matters more than Grok Build source display: OpenAI makes full Web Search sources opt-in through `include = ["web_search_call.action.sources"]`, while DeepSeek currently accepts that hint but does not return the requested sources; its current Responses API can therefore complete searches without returning source URLs. hellogrok still sends the standard hint so future provider support takes effect automatically. Native Responses and Chat authenticate with Bearer. Messages uses the documented `https://api.deepseek.com/anthropic` SDK base URL (the effective endpoint is `/anthropic/v1/messages`) and `X-Api-Key`; the `[1m]` suffix is accepted only as DeepSeek's documented Anthropic integration alias and must not be used with Responses.
